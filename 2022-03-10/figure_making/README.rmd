...
ggplot(data=TEX,mapping=aes(x=`Depth mbsf`,y=`TEX`))+
  ...
  labs(title="plot_sample_1",x="Depth (mbsf)", y=expression(TEX[86]))+
...
```
```{r}
#get data in right order for area plot AGE
...
tb_area_age <- Dino %>% 
  ...
  mutate(remainder = 100 - sum(c_across(-age))) %>% 
  drop_na() %>% 
  pivot_longer(-age, names_to = "taxa", values_to = "percent")
...
ggplot(data=tb_area_age, mapping=aes(x = age , y = percent, fill = taxa)) +
  geom_area(position = "stack", orientation = "x") +
  geom_segment(alpha=0.1,aes(y=100,xend = age, yend=0)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(limits=c(10.5,17),breaks=seq(10.5,17,1.5),expand = c(0,0)) +
  labs(
    fill = "",
    y = "%",
    x = "Age (Ma)"
...
#vertical plotting
P1<-ggplot(data=TEX,mapping=aes(x=`age`,y=`TEX`))+
  geom_point(colour="red")+
  geom_line(linetype=2)+
  geom_ribbon(mapping=aes(ymin=(TEX-0.06),ymax=(TEX+0.06)), width=10,alpha=0.25,fill="blue")+
  coord_flip()+
  labs(x="Age (Ma)", y=expression(TEX[86]))+
  scale_x_continuous(trans="reverse",limits=c(17,10.5),breaks=seq(17,10.5,-1.5))+
  scale_y_continuous(limits=c(0.2,0.8))+
  theme_classic()+
  theme(axis.title=element_text(size=10,colour="black"),
        axis.text=element_text(size=10,colour="black"),
...
P2<-ggplot(data=TEX)+
  geom_line(mapping=aes(x=`age`,y=`Methan index`),colour="blue")+
  geom_line(mapping=aes(x=`age`,y=`GDGT2/GDGT3`),colour="red")+
  coord_flip()+
  labs(x="Age (Ma)", y="Methane Index")+
 scale_x_continuous(trans="reverse",limits=c(17,10.5),breaks=seq(17,10.5,-1.5))+
  scale_y_continuous(limits=c(0,1),sec.axis=sec_axis(~.*2,name="GDGT2/GDGT3"))+
  theme_classic()+
  theme(axis.title.x.bottom =element_text(size=10,colour="red"),
...
P3<-ggplot(data=tb_area_age, mapping=aes(x = age , y = percent, fill = taxa)) +
  geom_area(position = "stack", orientation = "x") +
  geom_segment(alpha=0.1,aes(y=100,xend = age, yend=0))+
  coord_flip()+
  scale_y_continuous(expand = c(0,0)) +
...
gtsplot <- GTS %>%
  ...
  filter(between(top, 10 , 17 )) %>%
  # rectangles for each period
  ggplot() +
  scale_y_reverse()+
  geom_rect(aes(xmin = start, xmax = end, ymin = top, ymax = bot, fill = col),
            show.legend = FALSE, col = "black") +
  # make sure that the fill colour is given by our hex colours
  scale_fill_identity() +
  # add period names
  geom_text(aes(y = meanage, x = meanwidth, label = name, size = fontsize * .5,
                ## angle = fontangle,
  ...
  labs(y = "Age (millions of years ago)") +
  theme(
    # remove gray panel
