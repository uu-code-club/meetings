...
tb_area <- Dino %>%
 ...
 mutate(
    grp = case_when(
      str_detect(taxa, dino_genera[["imp"]]) ~ "imp", 
      str_detect(taxa, dino_genera[["opr"]]) ~ "opr",
      TRUE ~ taxa
     )
    ) %>%  
 group_by(depth, grp) %>% 
 summarise(percent = sum(percent, na.rm = TRUE), .groups = "drop_last") %>%
 # calculate remainder
 mutate(percent = if_else(grp == "remainder", 100 - sum(percent), percent)) %>% 
 ungroup() %>% 
 mutate(taxa = recode(grp, !!!dino_genera), .keep = "unused")
